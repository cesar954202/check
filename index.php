  ...
  <body>
    ...
    echo" </div>
        ...
        <div class='col s4' >
            <form action = '#' method = 'post'> 
              ...
              <input class = 'btn grey darken-3'  type='submit'> 
            </form>
        </div> 
    </div>
    ";
    ...
    if ($resultServidores = $mysqli->query($sqlquerys))
    {
      if ($resultServidores->num_rows > 0)
      {
        ...
        { 
          ...
          if(($diasSinReiniciar > 1) && ($diasSinReiniciar < 30)){$colorDias = " green ";}
          if($diasSinReiniciar <= 1){$colorDias = " black ";}
    ...
    if ($result = $mysqli->query($sqlquery))
    {
      if ($result->num_rows > 0)
      {
        ...
        {
        $icono = "<a href='javascript:pausar$row->id_pantalla()'><i class='material-icons red'>priority_high</i></a>";//<i class='material-icons green accent-4'>check</i>
        //$icono = "<i class='material-icons red'>priority_high</i>";
        echo "
        <script> 
            function pausar$row->id_pantalla() 
            { 
                var statusConfirm = confirm('¿Quieres pausar alerta de $row->nombre?'); 
                if (statusConfirm == true) 
                { 
                    document.location.href= 'monitor/ponerPausa.php?id_pantalla=$row->id_pantalla'; 
                } 
        ...
        $diff = $date1->diff($date2);
          ...
          else
          {
            ...
            if ($row->pausar == 1)
            {
              $icono = "<a href='javascript:quitarpausar$row->id_pantalla()'><i class='material-icons blue accent-4 white-text'>pause</i></a> ";
              echo "
              <script> 
                  function quitarpausar$row->id_pantalla() 
                  { 
                      var statusConfirm = confirm('¿Quieres quitar la pausa de alerta de $row->nombre?'); 
                      if (statusConfirm == true) 
                      { 
                          document.location.href= 'monitor/quitarPausa.php?id_pantalla=$row->id_pantalla'; 
                      } 
